...
class MySqlDatabaseSetter extends ComponentBase
{
    ...
    protected function changeDatabase($data)
    {
        ...
        DbManager::_()->reInit(['database_list'=>$data], App::Root());
    }
...
EOT;
            ...
            $sure = Console::_()->readLines(['sure'=>'N'],"Setting Mor Database[Y/N] [{sure}]?");
    ...
    private function makeFromDsn($options)
    {
        ...
        $dsn = $options['dsn'];
        $data=substr($dsn,strlen('mysql:'));
        $a=explode(';',trim($data,';'));
        
        $t=array_map(function($v){return explode("=",$v);},$a);
